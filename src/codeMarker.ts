...
class WARoot {
    ...
    /**
     * Toggle a file as ignored.
     * @param uri the `uri` of the target file.
     * @param relativePath the relative path of the target file to this workspace root.
     * @returns A list of `uri`s to decorate and the relevant username.
     */
    toggleIgnored(uri: vscode.Uri, relativePath: string): [vscode.Uri[], string] {
        let relevantUsername = "";
        ...
        if (index > -1) {
            // if it exists, remove it
            const ignoredEntry = this.ignoredFiles.splice(index, 1);
            relevantUsername = ignoredEntry[0].author;
            urisToDecorate = this.checkIfAllSiblingFilesAreAudited(uri);
        } else {
            // if it doesn't exist, add it
    ...
    /**
     * Concatenates an array of IgnoredFiles to the IgnoredFiles of this workspace root.
     * @param files The array of ignored files to be concatenated.
     */
    concatIgnored(files: AuditedFile[]): void {
        this.ignoredFiles = this.ignoredFiles.concat(files);
    }

    /**
     * Remove the IgnoredFiles of this workspace root for a specific username.
     * @param username The username whose IgnoredFiles entries need to be removed.
     */
    filterIgnored(username: string): void {
        this.ignoredFiles = this.ignoredFiles.filter((entry) => entry.author !== username);
    }

    /**
     * Checks whether the file at a particular path is in the IgnoredFiles of this workspace root.
     * @param path The path of the file to be checked.
     * @returns `true` if the file is in the IgnoredFiles, `false` if not.
     */
    isIgnored(path: string): boolean {
        return this.ignoredFiles.findIndex((entry) => entry.path === path) !== -1;
    }
    ...
    checkIfAllSiblingFilesAreAudited(uri: vscode.Uri): vscode.Uri[] {
        ...
        for (const file of files) {
            ...
            if (this.auditedFiles.findIndex((f) => f.path === relativePath) === -1 && this.ignoredFiles.findIndex((f) => f.path === relativePath) === -1) {
...
class MultiRootManager {
    ...
    /**
     * Given the `uri` of the current file, finds the corresponding workspace root and toggles the file as ignored.
     * @param uri The `uri` of the current file.
     * @returns A list of `uri`s to be decorated and a list of relevant usernames
     * (or `undefined` and "" if the `uri` is not in any workspace root.)
     */
    toggleIgnored(uri: vscode.Uri): [vscode.Uri[] | undefined, string[]] {
        const [closestRoot, closestRelativePath, inMultipleRoots] = this.getCorrespondingRootAndPath(uri.fsPath);
        if (closestRoot === undefined) {
            vscode.window.showErrorMessage(`weAudit: Error marking a file as ignored. The file at ${uri.fsPath} is not in any workspace root.`);
            // This file was not in any workspace root. No URIs to update.
            return [undefined, []];
        }
        ...
        if (!inMultipleRoots) {
            // Only in one workspace root: default behavior
            const [urisToDecorate, relevantUsername] = closestRoot.toggleIgnored(uri, closestRelativePath);
            return [urisToDecorate, [relevantUsername]];
        } else {
            // In multiple workspace roots: handle all of them
            const allRootsAndPaths = this.getAllCorrespondingRootsAndPaths(uri.fsPath);
            let isIgnored = false;
            const urisToDecorateMultiple: vscode.Uri[] = [];
            const relevantUsernamesMultiple: string[] = [];

            // Check if the file is ignored anywhere and remove it from there
            for (const [wsRoot, relativePath] of allRootsAndPaths) {
                if (wsRoot.isIgnored(relativePath)) {
                    isIgnored = true;
                    const [urisToAdd, relevantUsernameToAdd] = wsRoot.toggleIgnored(uri, relativePath);
                    urisToDecorateMultiple.push(...urisToAdd);
                    relevantUsernamesMultiple.push(relevantUsernameToAdd);
                }
            }

            // If it was not ignored anywhere, toggle it everywhere
            if (!isIgnored) {
                for (const [wsRoot, relativePath] of allRootsAndPaths) {
                    const [urisToAdd, relevantUsernameToAdd] = wsRoot.toggleIgnored(uri, relativePath);
                    urisToDecorateMultiple.push(...urisToAdd);
...
export class CodeMarker implements vscode.TreeDataProvider<TreeEntry> {
    ...
    /**
     * Toggles the current active file as ignored or not ignored.
     * Fires the onDidChangeFileDecorationsEmitter event if applicable.
     */
    toggleIgnored(): void {
        const editor = vscode.window.activeTextEditor;
        ...
        const [urisToDecorate, relevantUsernames] = this.workspaces.toggleIgnored(uri);
        if (urisToDecorate !== undefined) {
            for (const uriToDecorate of urisToDecorate) {
                this._onDidChangeFileDecorationsEmitter.fire(uriToDecorate);
            }
    ...
    loadSavedDataFromConfig(config: ConfigurationEntry, update: boolean, add: boolean): FullSerializedData | undefined {
        ...
        if (update) {
            if (add) {
                ...
                // handle older versions of the extension that don't have ignored files
                if (fullParsedEntries.ignoredFiles !== undefined) {
                    wsRoot.concatIgnored(fullParsedEntries.ignoredFiles);
                }
                ...
                wsRoot.filterAudited(config.username);
                wsRoot.filterIgnored(config.username);
                wsRoot.filterPartiallyAudited(config.username);
                this.resolvedEntries = this.resolvedEntries.filter(
                    (entry) =>
                        entry.author !== config.username ||
    ...
    provideFileDecoration(uri: vscode.Uri): vscode.FileDecoration | undefined {
        ...
        for (const [wsRootCheck, relativePathCheck] of allRootsAndPaths) {
            if (wsRootCheck.isIgnored(relativePathCheck)) {
                return {
