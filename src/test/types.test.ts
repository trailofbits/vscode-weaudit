...
describe("types.ts", () => {
    describe("createDefaultSerializedData", () => {
        it("should create empty serialized data with all required fields", () => {
            ...
            assert.deepStrictEqual(data.ignoredFiles, []);
    ...
    describe("validateSerializedData", () => {
        ...
        it("should return true for valid serialized data with ignoredFiles", () => {
            const data = createValidSerializedData();
            data.ignoredFiles = [{ path: "src/ignored.ts", author: "testuser" }];
            assert.strictEqual(validateSerializedData(data), true);
        });

        it("should handle data without ignoredFiles (backwards compatibility)", () => {
            const data = createValidSerializedData();
            delete data.ignoredFiles;
            assert.strictEqual(validateSerializedData(data), true);
        });

        it("should return false for ignored file missing path", () => {
            const data = createValidSerializedData();
            data.ignoredFiles = [{ author: "testuser" } as unknown as AuditedFile];
            assert.strictEqual(validateSerializedData(data), false);
        });

        it("should return false for ignored file missing author", () => {
            const data = createValidSerializedData();
            data.ignoredFiles = [{ path: "src/ignored.ts" } as unknown as AuditedFile];
            assert.strictEqual(validateSerializedData(data), false);
        });
